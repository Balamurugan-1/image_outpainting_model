...
def get_optimizer(params, name, **kwargs):
    if name == 'adam':
        ...
        # ~halves optimizer-state memory (int8 momentum buffers instead of fp32) at negligible
        # accuracy cost — see improvement.md §5.6 for why this was needed on this GPU.
...
def cosine_warmup_lr_scheduler(optimizer, warmup_steps=1000, total_steps=100000, min_lr_ratio=0.01):
    """Linear warmup to the optimizer's base LR, then cosine decay to min_lr_ratio * base LR by total_steps."""
    from torch.optim.lr_scheduler import LambdaLR
    import math
    def fn(step):
...
def get_lr_scheduler(optimizer, name, **kwargs):
    if name == 'customized':
        ...
        return cosine_warmup_lr_scheduler(optimizer, **kwargs)
    else:
        raise NotImplementedError(name)
...
def initialize_train_state(config, device):
    ...
    # EMA copy doubles the model's GPU memory for a quality benefit that only matters at eval/inference
    # time. On memory-constrained hardware, skipping it (use_ema=False) frees ~1.16GB. See improvement.md §5.6.
    use_ema = config.train.get('use_ema', True)
    nnet_ema = get_nnet(**config.nnet) if use_ema else None
    if nnet_ema is not None:
        nnet_ema.eval()
