...
class Intel(DatasetFactory):
    def __init__(self, path, resolution, embed_dim, grid_size):
        super().__init__()

        print(f'Counting Intel files from {path}')
        train_files = _list_image_files_recursively(path)
        class_names = [os.path.basename(os.path.dirname(file_path)) for file_path in train_files]
        sorted_classes = {x: i for i, x in enumerate(sorted(set(class_names)))}
        train_labels = [sorted_classes[x] for x in class_names]
        print('Finish counting Intel files, total images: %d' % len(train_files))
